...
class MultiHeadAttention(Module):
    ...
    def forward(self, Q: Tensor, K: Tensor, V: Tensor, key_padding_mask=None,attn_mask=None) -> Tensor:
        """
        ...
        K = self.k_linear(K)
        V = self.v_linear(V)
        Q = Q.view(Q.shape[0], Q.shape[1], self.num_heads, self.hidden_size)
        Q = Q.permute(0, 2, 1, 3)
        K = K.view(K.shape[0], K.shape[1], self.num_heads, self.hidden_size)
        K = K.permute(0, 2, 1, 3)
        V = V.view(V.shape[0], V.shape[1], self.num_heads, self.hidden_size)
        V = V.permute(0, 2, 1, 3)
        if attn_mask!=None:
            mask=attn_mask.float()*(-1e10)
        elif key_padding_mask!=None:
            mask=key_padding_mask.float()*(-1e10)
        else:
            mask=None
        Scaled_Product = self.ScaledDotProduct(Q, K, V, mask)  # shape:bzs,num_head,seq_len_q,hidden_size
        batch_size = Q.shape[0]
        seq_len = Q.shape[2]
        Scaled_Product = Scaled_Product.transpose(2, 1)
        Scaled_Product = self.dense_linear(Scaled_Product.reshape(batch_size, seq_len, self.features))
...
if __name__ == '__main__':
    ...
    #expect [[550,5.5]]
    print(multihead.ScaledDotProduct(tmp_q,test_k,test_v,None))
